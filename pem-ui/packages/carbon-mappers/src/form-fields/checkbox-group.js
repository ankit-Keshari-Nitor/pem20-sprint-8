...
import { FORM_FIELD_GROUPS, FORM_FIELD_LABEL, FORM_FIELD_TYPE, helperText, NameLabel, options, isRequired, labelText, readOnly } from '../constant';
...
const CheckboxGroup = ({ field, id, currentPath, onChangeHandle, previewMode }) => {
  const { type, labelText, value, label, options, isRequired, helperText, ...rest } = field;
  ...
  return (
    <CarbonCheckboxGroup data-testid={`${id}-${previewMode}`} id={`${id}-${previewMode}`} legendText={labelText === undefined ? label : labelText} helperText={helperText}>
      {options &&
        options.map((element) => {
...
CheckboxGroup.config = {
  ...
  icon: <CheckboxIcon />,
  editableProps: {
    Basic: [NameLabel, labelText, helperText, options, readOnly],
    Condition: []
  },
