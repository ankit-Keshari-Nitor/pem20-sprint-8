...
public class ActivityController {
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", content = {
                    @Content(schema = @Schema(implementation = ActivityDefnUpdateResp.class), mediaType = MediaType.APPLICATION_JSON_VALUE),
                    @Content(schema = @Schema(implementation = ActivityDefnUpdateResp.class), mediaType = MediaType.APPLICATION_XML_VALUE) }),
    ...
    public ResponseEntity<Object> updateActivityDefinitionByKey(@RequestPart(value = "name", required = true) @Size(min = 1, max = 80) String name,
        ...
        ActivityDefnUpdateResp activityDefnUpdateResp = activityDefnService.updateActivityDefinitionByKey(sponsorContext,name, description, activityDefnKey);
        return new ResponseEntity<>(activityDefnUpdateResp,HttpStatus.OK);
    }
}
