...
class ActivityVersionControllerTest {
    ...
    void testPostCreateActivityDefnVersion() throws SQLException, IOException, OnlyOneDraftVersionException {
        ...
        ResponseEntity<Object> resp = ResponseEntity.ok().build();
        Mockito.when(activityVersionService.createActivityDefnVersion(Mockito.anyString(),Mockito.anyString(),Mockito.any(MultipartFile.class),Mockito.anyBoolean(),Mockito.anyString(),Mockito.any(HttpServletRequest.class)))
                .thenReturn(resp);
