...
const CheckboxGroup = ({ field, id, currentPath, onChangeHandle, previewMode }) => {
  ...
  return (
    <CarbonCheckboxGroup readOnly={readOnly} orientation={orientation ? 'horizontal' : 'vertical'} data-testid={`${id}-${previewMode}`} id={`${id}-${previewMode}`} legendText={labelText === undefined ? label : labelText} helperText={helperText}>
      {options &&
        options.map((element) => {
